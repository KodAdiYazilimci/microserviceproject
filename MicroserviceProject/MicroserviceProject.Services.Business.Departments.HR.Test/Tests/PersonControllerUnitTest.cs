...
namespace MicroserviceProject.Services.Business.Departments.HR.Test.Tests
{
    ...
    public class PersonControllerUnitTest
    {
        ...
        {
            IActionResult createPersonResult = await personController.CreatePerson(new Model.Department.HR.PersonModel()
            {
        ...
        {
            IActionResult createTitleResult = await personController.CreateTitle(new Model.Department.HR.TitleModel
            {
                Name = new Random().Next(0, int.MaxValue).ToString()
            }, cancellationTokenSource);

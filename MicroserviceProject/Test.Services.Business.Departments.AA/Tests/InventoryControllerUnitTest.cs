...
using Services.Business.Departments.AA.Controllers;
...
using Test.Services.Business.Departments.AA.Factories.Services;

namespace Test.Services.Business.Departments.AA.Tests
{
    [TestClass]
        ...
        public async Task CreateDefaultInventoryForNewWorkerTest()
        {
            ServiceResultModel<List<InventoryModel>> inventoriesResult =
                await serviceCommunicator.Call<List<InventoryModel>>(
                    serviceName: routeNameProvider.AA_GetInventories,
                    postData: null,
                    queryParameters: null,
                    headers: null,
                    cancellationTokenSource: cancellationTokenSource);

    ...
    public class PersonService : BaseService, IRollbackableAsync<int>, IDisposable, IDisposableInjections
    {
        ...
        public async Task<int> CreateWorkerAsync(WorkerModel worker, CancellationTokenSource cancellationTokenSource)
        {
            ...
            if (worker.AAInventories == null)
                worker.AAInventories = new List<MicroserviceProject.Infrastructure.Communication.Model.Department.AA.InventoryModel>();

            if (!worker.AAInventories.Any())
            {
                ServiceResultModel<List<MicroserviceProject.Infrastructure.Communication.Model.Department.AA.InventoryModel>> defaultInventoriesServiceResult =
                    await _serviceCommunicator.Call<List<MicroserviceProject.Infrastructure.Communication.Model.Department.AA.InventoryModel>>(
            ...
            if (worker.ITInventories == null)
                worker.ITInventories = new List<MicroserviceProject.Infrastructure.Communication.Model.Department.IT.InventoryModel>();

            if (!worker.ITInventories.Any())
            {
                ServiceResultModel<List<MicroserviceProject.Infrastructure.Communication.Model.Department.IT.InventoryModel>> defaultInventoriesServiceResult =
                    await _serviceCommunicator.Call<List<MicroserviceProject.Infrastructure.Communication.Model.Department.IT.InventoryModel>>(
